...
cl = LINETCR.LINE()
cl.login(token="EotNM1LHLnvoR4AMjlP9.dKVw3xpTsB0QvQroj0VpIq.SmRXdaCrwPPx6pxRwf15ugjB26Ms9O+odc/6ot8dMJI=")
cl.loginResult()

ki = LINETCR.LINE()
ki.login(token="EoK86ZPIGHol6oWTabVe.belVJ7MCwbwkSXaAOYZ4hG.NtOwtrPZBiJoMOSPa3+hNbhEkBsWit8cI9yYJuqo3ks=")
ki.loginResult()

kk = LINETCR.LINE()
kk.login(token="Eo7kU3Ce0SCn9SvHDVmc.kT6iR169jqOfCEbXxwHDVa.CARLMlz82anzbChOVd1Webwb4/IkOvp501aT3YyMhrM=")
kk.loginResult()

kc = LINETCR.LINE()
kc.login(token="EoBaISYNfFFEmkjkraPa.RQCE9wL3Mq5c3Ev1yfPIsG.WJKW0M3kB48MUQRAa8EdR4lU6PbjH5C/hdXxLIDkSow=")
kc.loginResult()
...
¦
+--------------------------------
¦ PERINTAH SPESIAL UNTUK STAFF  ¦
+--------------------------------
¦
¦[????Mask??] cancl on/off
¦[????Mask??] Protection on/off
¦[????Mask??] Tagall
¦[????Mask??] idline
¦[????Mask??] instagram
¦[????Mask??] Buka
...
def bot(op):
	...
	if op.type == 55:
		...
		if msg.from_ in admin:
			...
			jml = 0
                ...
                if msg.from_ in owner:
                        ...
                        h = ""
                        for i in gid:
                            h += "? %s  \n" % (cl.getGroup(i).name + " ?? _ [ " + str(len (cl.getGroup(i).members))+" ]")
                        cl.sendText(msg.to, "     ? [ ?List Grup? ] ?\n"+ h +"Total Group _" +"[ "+str(len(gid))+" ]")
		...
		if msg.from_ in admin:
            ...
            elif "Listharga: " in msg.text:
                wait["message"] = msg.text.replace("Listharga: ","")
                cl.sendText(msg.to,"Listharga Be Changed")
